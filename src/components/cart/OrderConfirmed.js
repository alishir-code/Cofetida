...
export function OrderConfirmed({
  ...
  //close the OrderConfirmed
  const handleClosePopup = function (e) {
    if (!e.target.closest("#popup-confirm")) setIsOpenOrderConfirmed(false);
  };
  return (
    <div onClick={handleClosePopup} className="bgDivToClosePopup">
      <div
        id="popup-confirm"
        className=" col-12 col-sm-7 col-lg-4 orderConfirmed p-3 rounded-3"
      >
        <svg
          width="48"
          height="48"
        ...
        <h4 style={{ fontWeight: "bold" }}>لیست دسر های که شفارش دادین</h4>
        <p>ما امیدوار هستیم که از دسر ها لذت ببرید</p>
        <div className="divitemInPopupContainer rounded-3">
          <div className="itemInPopupContainer">
            {card.map((cardItem) => (
              <Cartitempopup card={cardItem} key={cardItem.id} />
            ))}
          </div>
          <CartFoother
            card={card}
